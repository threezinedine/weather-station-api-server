...
class UserControllerTest(unittest.TestCase):
    ...
    def test_given_no_user_is_created_when_create_a_new_user_then_return_ok_and_None(self):
        ...
        assertUser(user, self.first_testing_username, self.testing_password)
    ...
    def test_given_a_user_is_created_when_querying_user_by_username_then_that_returns_ok_and_that_user(self):
        ...
        assertStatus(status, HTTP_200_OK)
        assertUser(user, username=self.first_testing_username, password=self.testing_password)
    ...
    def test_given_a_user_is_created_when_asking_all_users_then_returns_the_list_contains_that_user(self):
        ...
        assertStatus(status, HTTP_200_OK)
        assert len(users) == 1
        assertUser(users[0], self.first_testing_username, self.testing_password)
    ...
    def test_given_two_users_are_created_when_asking_by_id_that_is_existed_then_returns_ok_and_user(self):
        ...
        assertStatus(first_status, HTTP_200_OK)
        assertStatus(second_status, HTTP_200_OK)

        assertUser(first_user, self.first_testing_username, self.testing_password)
        assertUser(second_user, self.second_first_testing_username, self.second_testing_password)
    ...
    def test_given_a_user_is_created_when_querying_user_by_valid_username_and_invalid_password_then_returns_password_is_wrong_and_none(self):
        ...
        assertStatus(status, WRONG_PASSWORD_STATUS_CODE, WRONG_PASSWORD_DETAIL)
        assert user is None
    ...
    def test_given_a_user_is_created_when_change_the_name_of_existed_username_then_returns_ok_and_that_user(self):
        ...
        assertStatus(status, HTTP_200_OK)
        assertUser(user, self.testing_changed_username, self.testing_new_user_password)
        _, modified_user = self.user_controller.get_user_by_name(self.testing_changed_username)
        assertUser(modified_user, self.testing_changed_username, self.testing_new_user_password)
    ...
    def test_given_a_user_is_created_when_change_the_password_then_returns_ok_and_that_user(self):
        ...
        assertStatus(status, HTTP_200_OK)
        assertUser(user, self.first_testing_username, self.testing_changed_password)
        _, modified_user = self.user_controller.get_user_by_name(self.first_testing_username)
        assertUser(modified_user, self.first_testing_username, self.testing_changed_password)
    ...
    def test_given_a_user_is_created_when_change_both_valid_username_and_password_then_returns_ok_and_that_user(self):
        ...
        assertStatus(status, HTTP_200_OK)
        assertUser(user, self.testing_changed_username, self.testing_changed_password)
        _, modified_user = self.user_controller.get_user_by_name(self.testing_changed_username)
        assertUser(modified_user, self.testing_changed_username, self.testing_changed_password)
    ...
    def test_given_two_users_are_created_when_deleting_that_user_by_valid_username_then_returns_ok_and_none(self):
        ...
        assertUser(remain_users[0], self.second_first_testing_username, self.second_testing_password)
    ...
    def test_given_two_user_are_created_when_deleting_user_by_invalid_username_then_returns_username_does_not_existed_exception_and_none(self):
        ...
        assertUser(remain_users[0], self.first_testing_username, self.testing_password)
        assertUser(remain_users[1], self.second_first_testing_username, self.second_testing_password)
